...
/*******************************************************************************
* Function Name: MS5607_read()
****************************************************************************//**
* \brief
*   Read a register over the MS5607 I2C bus
*
* \param state [in/out]
* Pointer to the state struct
*
...
*  Error code of the operation
*******************************************************************************/
uint32_t MS5607_read(MS5607_STATE_S *const state, uint8_t regAddr, uint8_t* val){
  /* Ensure Comms exists */
  uint32_t error = Comms_validateI2C(state->_i2c);
  if(error){
    /* Store the comms error reported */
  ...
  /* Proceed with write */
    ...
    uint8_t readArray[TWO];
    uint32_t i2cError = state->_i2c->readArray(state->_deviceAddr, regAddr, readArray, TWO);
    /* Write out */
    uint16_t MSDB = readArray[ZERO];
    uint16_t LSDB = readArray[ONE];
